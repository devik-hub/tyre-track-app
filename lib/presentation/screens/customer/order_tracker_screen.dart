...
// ═══════════════════════════════════════════════════════════════════════════
// BUG #6 FIX — Customer order tracker: real-time stream, no manual refresh
// ═══════════════════════════════════════════════════════════════════════════
...
class _OrderTrackerBody extends StatelessWidget {
        ...
        children: [
          ...
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              ...
              const Text('Live tracking active', style: TextStyle(color: Colors.green, fontSize: 12, fontWeight: FontWeight.w600)),
            ]),
          ),
            ...
            Container(
              ...
              child: Row(children: [
                ...
                Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  const Text('Order Cancelled', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.red)),
                  if (order.cancelledAt != null) Text('On ${_date(order.cancelledAt!)}', style: const TextStyle(color: Colors.grey, fontSize: 12)),
                ]),
              ]),
            ),
          ],
          ...
          _infoCard('Items', [
            ...order.items.map((item) => Padding(
              ...
              child: Row(children: [
                Expanded(child: Text('${item.productName.isNotEmpty ? item.productName : 'Item'} × ${item.quantity}', style: const TextStyle(fontSize: 14))),
                Text('₹${item.totalPriceRupees.toStringAsFixed(0)}', style: const TextStyle(fontWeight: FontWeight.bold, color: AppColors.mrfRed)),
              ]),
            )),
          ]),
  ...
  String _stepSubtitle(String status, OrderModel order) {
    switch (status) {
      case 'pending_confirmation': return order.createdAt != null ? _date(order.createdAt) : '';
      case 'confirmed':  return order.confirmedAt != null ? _date(order.confirmedAt!) : '';
      case 'shipped':    return order.shippedAt   != null ? _date(order.shippedAt!)   : '';
      case 'delivered':  return order.deliveredAt != null ? _date(order.deliveredAt!) : '';
      default:           return '';
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
      Column(children: [
        Container(
          width: 28, height: 28,
          decoration: BoxDecoration(color: done ? AppColors.mrfRed : Colors.grey.shade200, shape: BoxShape.circle,
      ...
      Expanded(child: Padding(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Text(label, style: TextStyle(fontWeight: isCurrent ? FontWeight.bold : FontWeight.normal, color: done ? Colors.black87 : Colors.grey.shade400, fontSize: 13)),
