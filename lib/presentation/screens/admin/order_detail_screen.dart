...
class _OrderDetailBody extends ConsumerWidget{
        ...
        children: [
          ...
          _SectionCard(
            ...
            child: customerAsync.when(
              ...
              error: (_, __) => _CustomerFallback(order: order),
              data: (user) => user != null
                    ...
                    Expanded(
                      child: Column(
                        ...
                        children: [
                          ...
                          Text('${item.category.toUpperCase()} × ${item.quantity}',
  ...
  Widget _amtRow(String label, double value, {bool highlight = false}) => Padding(
    ...
    child: Row(
      ...
      children: [
        Text(label,
          style: TextStyle(color: highlight ? Colors.white : Colors.grey.shade400, fontWeight: highlight ? FontWeight.bold : FontWeight.normal),
        ),
        Text('₹${value.toStringAsFixed(0)}',
          style: TextStyle(color: highlight ? AppColors.mrfRed : Colors.white70, fontWeight: highlight ? FontWeight.bold : FontWeight.normal, fontSize: highlight ? 18 : 14),
        ),
      ],
    ),
  );
          ...
          Expanded(
            child: Column(
              ...
              children: [
                ...
                Text('Order placed ${_date(order.createdAt)}',
                  style: TextStyle(color: Colors.grey.shade400, fontSize: 12),
                ),
              ],
            ),
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Text('CASH ON DELIVERY',
            style: TextStyle(color: order.isCodPending ? Colors.amber : Colors.green, fontWeight: FontWeight.bold, letterSpacing: 1),
          ),
        ],),
        const SizedBox(height: 8),
            ...
            child: Row(
              children: [
                ...
                if(order.collectedAt != null) ...[
                  const Spacer(),
                  Text('${order.collectedAt!.day}/${order.collectedAt!.month}/${order.collectedAt!.year}',
  ...
  void _confirmCollect(BuildContext context, WidgetRef ref, String? adminUid){
    showDialog(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text('Mark ₹${order.finalAmountRupees.toStringAsFixed(0)} as collected for order #${order.orderId.substring(0, 8).toUpperCase()}?',
              ...
              try{
                ...
                if(context.mounted){
                  ScaffoldMessenger.of(context).showSnackBar(
                    const SnackBar(
                      content: Text('Cash marked as collected'),
                    ),
                  );
                }
              } catch(e){
        ...
        children: [
          DropdownButtonFormField<String>(
            ...
            items: statuses.map((s) => DropdownMenuItem(
              value: s,
              child: Text(s.replaceAll('_', ' ').toUpperCase()))).toList(),
            onChanged: (v){
  ...
  Future<void> _updateStatus() async{
    ...
    try{
      ...
      if(mounted){
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Order status → ${_selected.replaceAll('_', ' ')}'),
          ),
        );
      }
    } catch(e){
...
class _CustomerInfo extends StatelessWidget{
  ...
  Widget build(BuildContext context) => Column(
    crossAxisAlignment: CrossAxisAlignment.start,
    children: [
      Text(name.isNotEmpty ? name : 'Unknown Customer',
