  ...
  Future<void> _handleCheckout(double totalRupees, int totalPaise, List<OrderItem> cartItems) async {
    ...
    razorpay.onSuccess = (response) async {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Order save failed: $e')));
          setState(() => _isProcessing = false);
        }
      }
    };
    ...
    razorpay.openCheckout(
      amount:      totalRupees,   // Razorpay service handles conversion internally
      contact:     user.phone,
      email:       user.email.isNotEmpty ? user.email : 'customer@jagadale.com',
            ...
            children: [
              ...
              Row(children: [
                Expanded(child: RadioListTile<String>(
                  title: const Text('Online', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
                  value: 'online',
                ...
                Expanded(child: RadioListTile<String>(
                  title: const Text('COD', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)),
                  value: 'cod',
                  groupValue: _selectedPaymentMethod,
                  activeColor: AppColors.mrfRed,
                  contentPadding: EdgeInsets.zero,
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: _isProcessing
                      ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: AppColors.mrfRed, strokeWidth: 2))
                      : Text(_selectedPaymentMethod == 'cod' ? 'Place Order' : 'Proceed to Payment'),
                ),
              ),
            ],
          ),
