  ...
  Future<void> _handleCheckout(double totalRupees, int totalPaise, List<OrderItem> cartItems) async {
    ...
    if (_selectedPaymentMethod == 'cod') {
      try {
        ...
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Order Placed! Cash on Delivery confirmed ✨')));
          context.go(AppRoutes.home);
          Future.delayed(const Duration(milliseconds: 300), () => ref.read(cartProvider.notifier).clear());
        }
        print('✅ COD order placed: $orderId');
      } catch (e) {
        if (mounted) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Order failed: $e'))); setState(() => _isProcessing = false); }
      }
      return;
    }
    ...
    razorpay.onSuccess = (response) async {
      try {
        ...
        if (mounted) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Order save failed: $e'))); setState(() => _isProcessing = false); }
      }
    };
    razorpay.onFailure = (response) {
      if (mounted) { ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('Payment Cancelled/Failed: ${response.message}'))); setState(() => _isProcessing = false); }
    };
    ...
    return Scaffold(
                ...
                return Card(
                  ...
                  child: Padding(padding: const EdgeInsets.all(16.0), child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(item.productName.isNotEmpty ? item.productName : 'Product', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                      const SizedBox(height: 4),
                      Text('₹${item.unitPriceRupees.toStringAsFixed(0)}', style: const TextStyle(color: AppColors.mrfRed, fontWeight: FontWeight.bold)),
                    ])),
                    Row(children: [
      ...
      bottomNavigationBar: cartItems.isEmpty ? null : SafeArea(child: Container(
        ...
        child: Column(mainAxisSize: MainAxisSize.min, crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Row(children: [
            Expanded(child: RadioListTile<String>(title: const Text('Online', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)), value: 'online', groupValue: _selectedPaymentMethod, activeColor: AppColors.mrfRed, contentPadding: EdgeInsets.zero, dense: true, visualDensity: const VisualDensity(horizontal: -4, vertical: -4), onChanged: (val) => setState(() => _selectedPaymentMethod = val!))),
            Expanded(child: RadioListTile<String>(title: const Text('COD', style: TextStyle(fontSize: 13, fontWeight: FontWeight.bold)), value: 'cod', groupValue: _selectedPaymentMethod, activeColor: AppColors.mrfRed, contentPadding: EdgeInsets.zero, dense: true, visualDensity: const VisualDensity(horizontal: -4, vertical: -4), onChanged: (val) => setState(() => _selectedPaymentMethod = val!))),
          ]),
          const Divider(height: 24),
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
            const Text('Total Amount', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
            Text('₹${totalRupees.toStringAsFixed(0)}', style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.mrfRed)),
          ]),
          const SizedBox(height: 16),
          SizedBox(width: double.infinity, child: ElevatedButton(
            onPressed: () { if (_isProcessing) return; _handleCheckout(totalRupees, totalPaise, cartItems); },
            child: _isProcessing ? const SizedBox(height: 20, width: 20, child: CircularProgressIndicator(color: AppColors.mrfRed, strokeWidth: 2)) : Text(_selectedPaymentMethod == 'cod' ? 'Place Order' : 'Proceed to Payment'),
          )),
        ]),
      )),
    );
