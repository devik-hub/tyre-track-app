...
class _HomeScreenState extends ConsumerState<HomeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        leading: Padding(padding: const EdgeInsets.all(8.0), child: Image.asset('assets/images/logo.png', errorBuilder: (c, e, s) => const Icon(Icons.tire_repair))),
        title: const Text('Jagadale Retreads'),
        actions: [
          IconButton(icon: const Icon(Icons.search), onPressed: () => showSearch(context: context, delegate: ProductSearchDelegate(ref))),
  ...
  Widget _buildGreetingBanner(BuildContext context, String name) {
    ...
    return Container(
      ...
      decoration: const BoxDecoration(gradient: LinearGradient(colors: [AppColors.mrfRed, AppColors.mrfDarkRed], begin: Alignment.topLeft, end: Alignment.bottomRight)),
  ...
  Widget _buildQuickActions(BuildContext context) {
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0), child: GridView.count(
  ...
  Widget _buildServicesSection(BuildContext context) {
    final services = [
      {'name': 'Retreading', 'icon': Icons.autorenew}, {'name': 'Remoulding', 'icon': Icons.handyman},
      {'name': 'Inspection', 'icon': Icons.search}, {'name': 'New Fitment', 'icon': Icons.build_circle},
    ];
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 100, child: ListView.builder(scrollDirection: Axis.horizontal, itemCount: services.length, itemBuilder: (context, index) {
        return InkWell(onTap: () => context.go(AppRoutes.services), child: Container(width: 100, margin: const EdgeInsets.only(right: 12), child: Card(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          Icon(services[index]['icon'] as IconData, color: AppColors.mrfRed), const SizedBox(height: 8),
  ...
  Widget _buildFeaturedTyres(BuildContext context, WidgetRef ref) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 200, child: productsAsync.when(
        ...
        data: (products) {
          ...
          if (display.isEmpty) return const Center(child: Text('No products yet.', style: TextStyle(color: Colors.grey)));
  ...
  Widget _buildFeaturedCasings(BuildContext context, WidgetRef ref) {
    ...
    return Padding(padding: const EdgeInsets.symmetric(horizontal: 16.0), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      SizedBox(height: 200, child: casingsAsync.when(
        loading: () => const Center(child: CircularProgressIndicator()),
        error: (e, _) => const SizedBox.shrink(),
        data: (casings) {
          if (casings.isEmpty) return Center(child: Text('No featured casings yet.', style: TextStyle(color: Colors.grey.shade400)));
  ...
  Widget _buildProductRow(BuildContext context, List products) {
    return ListView.builder(scrollDirection: Axis.horizontal, itemCount: products.length, itemBuilder: (context, index) {
      final p = products[index];
      return InkWell(onTap: () => context.push(AppRoutes.tyreDetail, extra: p.productId), child: Container(width: 160, margin: const EdgeInsets.only(right: 12), child: Card(clipBehavior: Clip.antiAlias, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Container(height: 100, color: Colors.grey[200], child: Center(child: p.imageUrls.isNotEmpty
  ...
  Widget _buildMiniAction(BuildContext context, IconData icon, String title, VoidCallback onTap) {
    return InkWell(onTap: onTap, borderRadius: BorderRadius.circular(8), child: Column(children: [
